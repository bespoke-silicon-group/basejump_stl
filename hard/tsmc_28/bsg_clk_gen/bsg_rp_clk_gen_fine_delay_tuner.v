...
module bsg_rp_clk_gen_fine_delay_tuner
  (input i
   ...
   CKND3BWP30P140 I2_1 (.I(ft[1]),.ZN());
   // synopsys rp_orient ({N FS} I3_1)
   CKND3BWP30P140 I3_1 (.I(ft[2]),.ZN());
   // synopsys rp_orient ({N FS} I3_2)
   CKND4BWP30P140 I3_2 (.I(ft[2]),.ZN());
   // synopsys rp_orient ({N FS} I4_1)
   CKND3BWP30P140 I4_1 (.I(ft[3]),.ZN());
   // synopsys rp_orient ({N FS} I4_2)
   CKND4BWP30P140 I4_2 (.I(ft[3]),.ZN());
   // synopsys rp_orient ({N FS} I4_3)
   CKND4BWP30P140 I4_3 (.I(ft[3]),.ZN());

   // same driver with different caps and thus different transition times
   // synopsys rp_fill (1 0 UX)
   CKND4BWP30P140 I0 (.I(i), .ZN(i_inv));     // decouple load of FDT from previous stage; also makes this inverting
   CKND2BWP30P140 I1 (.I(i_inv), .ZN(ft[0]));
   CKND2BWP30P140 I2 (.I(i_inv), .ZN(ft[1]));
   CKND2BWP30P140 I3 (.I(i_inv), .ZN(ft[2]));
   CKND2BWP30P140 I4 (.I(i_inv), .ZN(ft[3]));
   ...
   DFCND4BWP30P140 DFFR1 (.D(mux_lo[0]),.CP(o), .Q (sel_r[0]), .QN(), .CDN(async_reset_neg_i));

   MUX4ND4BWP30P140 M2 (.I0(ft[3]), .I1(ft[2]), .I2(ft[1]), .I3(ft[0])
              ,.S0(sel_r[0]), .S1(sel_r[1])
              ,.ZN(o)
              );

   // capture on positive edge
   DFCND4BWP30P140 DFFR2 (.D(mux_lo[1]),.CP(o), .Q (sel_r[1]), .QN(), .CDN(async_reset_neg_i));
   MUX2D1BWP30P140   MX2 (.I0(sel_r [1]),.I1  (sel_i[1]), .S(we_i)    ,.Z(mux_lo[1]         ));

   // synopsys rp_fill (3 2 UX)

   CKBD8BWP30P140 ICLK (.I(o),        .Z(buf_o) );
