...
module bsg_rp_clk_gen_atomic_delay_tuner
  (input i
   ...
   CKND2BWP30P140 I4b (.I(signal[3]), .ZN()          ); // this is an extra gate because
   ...
   CKND4BWP30P140 I8a (.I(signal[7]), .ZN()          );
   CKND3BWP30P140 I8b (.I(signal[7]), .ZN()          ); // fudge factor capacitance
   ...
   MUX4ND4BWP30P140 M1 ( .I0(signal[8])
                        ,.I1(signal[6])
                        ,.I2(zero_bit)
                        ,.I3(signal[0])
   ...
   IND2D2BWP30P140 NB (.A1(sel_r[0]), .B1(async_reset_neg_i), .ZN(sel_r[1]));
   TIELBWP30P140   ZB (.ZN(zero_bit));

   DFCND4BWP30P140 sel_r_reg_0 (.D(mux_lo[0]), .CP(o)      ,.CDN(async_reset_neg_i), .Q(sel_r[0]), .QN());
   //LHCND4BWP30P140 sel_r_latch_0 (.D(mux_lo[0]), .E(o)      ,.CDN(async_reset_neg_i), .Q(sel_r[0]), .QN());
   ...
   MUX2D1BWP30P140 MX1          ( .I0(sel_i), .I1(sel_r[0]), .S(we_i_sync_sync_nand), .Z(mux_lo[0]));

   // nand 10ps versus 22ps
   ND2D1BWP30P140 bsg_we_nand   (.A1(we_i_sync_sync), .A2(we_inited_i), .ZN(we_i_sync_sync_nand));
   // synchronizer flops; negative edge triggered
   //DFND1BWP30P140 bsg_SYNC_2_r  (.D(we_i_sync), .CPN(o), .Q(we_i_sync_sync), .QN());
   //DFND1BWP30P140 bsg_SYNC_1_r  (.D(we_async_i),     .CPN(o), .Q(we_i_sync),      .QN());
   DFNCND1BWP30P140 bsg_SYNC_2_r  (.D(we_i_sync),  .CPN(o), .CDN(async_reset_neg_i), .Q(we_i_sync_sync), .QN());
   DFNCND1BWP30P140 bsg_SYNC_1_r  (.D(we_async_i), .CPN(o), .CDN(async_reset_neg_i), .Q(we_i_sync),      .QN());
   // drive we signal to next CDT; minimize capacitive load on critical we_i path
   INVD0BWP30P140 we_o_pre      (.I(we_i_sync_sync_nand), .ZN(we_o_pre_buf));
   BUFFD4BWP30P140 we_o_buf     (.I(we_o_pre_buf),. Z(we_o));
