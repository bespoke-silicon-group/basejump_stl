...
proc bsg_link_sdr_out_generated_clock_constraints {clk_name ports max_delay min_delay} {
  ...
  set_output_delay -min $min_delay -clock $clk_name $ports -add_delay
}
...
proc bsg_link_sdr_constraints { \
  ...
  # in token
  set in_tkn_clk_period         [expr 2*$in_clk_period]
  ...
  #set_driving_cell -no_design_rule -lib_cell "CKBD4BWP7T40P140" $in_tkn_clk_port

  # out token
  set out_tkn_clk_period         [expr 2*$out_clk_period]
  ...
  #set_driving_cell -no_design_rule -lib_cell "CKBD4BWP7T40P140" $out_tkn_clk_port
  ...
  if {$BSG_LINK_SDR_USE_GENERATED_CLOCK == 0} {
    ...
    # This clock doesn't time anything, but it identifies it as a clock
    create_clock -period $out_clk_period -name $out_clk_name $out_clk_port
    set_clock_group -async -group [get_clocks ${out_clk_name}]
  } else {
    set max_output_delay       [expr ($out_clk_period/2)-$out_clk_margin]
    set min_output_delay       [expr $out_clk_margin-($out_clk_period/2)]
    create_generated_clock -divide_by 1 -invert -master_clock $master_clk_name -source $master_clk_port -name $out_clk_name -add $out_clk_port
