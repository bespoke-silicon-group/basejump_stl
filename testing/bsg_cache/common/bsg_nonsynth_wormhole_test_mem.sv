...
module bsg_nonsynth_wormhole_test_mem
  ...
  assign header_flit_out.uncached_op = is_io_mem_p;
        ...
        if (wh_link_sif_in.v) begin
          ...
          case (opcode_r)
            e_cache_wh_read: begin
              mem_state_n = SEND_FILL_HEADER;
              if (is_io_mem_p && zero_latency_p) begin
                wh_link_sif_out.v = 1'b1;
                wh_link_sif_out.data = header_flit_out;
                mem_state_n = wh_link_sif_in.ready_and_rev ? SEND_FILL_DATA : SEND_FILL_HEADER;
              end
            end
  ...
  if (no_coordination_p) begin
    ...
    ? mem_addr_width_lp'(addr_r >> byte_offset_width_lp)
